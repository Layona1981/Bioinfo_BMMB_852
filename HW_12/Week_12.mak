...
SRA_DATA_DIR = sra_data
VCF_OUTPUT = merged_variants.vcf  # Output file for merged VCF
CSV_OUTPUT = results_summary.csv     # Output file for CSV summary

# Targets
.PHONY: all genome simulate download index align call_variants merge_vcf generate_csv clean help

# Default target
all: genome download index align call_variants merge_vcf generate_csv
...
# Target to merge VCF files
merge_vcf: $(SRR1)_variants.vcf.gz $(SRR2)_variants.vcf.gz $(SRR3)_variants.vcf.gz
	@echo "Merging VCF files..."
	bcftools merge $(SRR1)_variants.vcf.gz $(SRR2)_variants.vcf.gz $(SRR3)_variants.vcf.gz -o $(VCF_OUTPUT)
	@echo "Merged VCF file created: $(VCF_OUTPUT)."
...
# Target to align reads to the genome
align: $(SRA_DATA_DIR)/$(SRR1).fastq $(SRA_DATA_DIR)/$(SRR2).fastq $(SRA_DATA_DIR)/$(SRR3).fastq
	@echo "Aligning reads to generate SAM files..."
	parallel "bwa mem $(GENOME_FASTA) {1} > {1/.fastq/.sam}" ::: $(SRA_DATA_DIR)/$(SRR1).fastq $(SRA_DATA_DIR)/$(SRR2).fastq $(SRA_DATA_DIR)/$(SRR3).fastq
	@echo "SAM files generated."

# Target to convert SAM to BAM
sam_to_bam:
	@echo "Converting SAM to BAM..."
	parallel "samtools view -bS {1} > {1/.sam/.bam}" ::: $(SRR1).sam $(SRR2).sam $(SRR3).sam
	@echo "BAM files generated."

# Target to sort BAM files
sort_bam:
	@echo "Sorting BAM files..."
	parallel "samtools sort {1} -o {1/.bam/.sorted.bam}" ::: $(SRR1).bam $(SRR2).bam $(SRR3).bam
	@echo "Sorted BAM files generated."

# Target to call variants from aligned reads
call_variants: sam_to_bam sort_bam
	@echo "Calling variants..."
	parallel "bcftools mpileup -f $(GENOME_FASTA) {1/.sorted.bam} | bcftools call -mv -o {1/.sorted.bam/_variants.vcf}" ::: $(SRR1).sorted.bam $(SRR2).sorted.bam $(SRR3).sorted.bam
	@echo "Variants called."

# Target to generate a CSV summary
generate_csv:
	@echo "Sample,Variants Called" > $(CSV_OUTPUT)
	@echo "$(SRR1), $$(grep -c '^#' $(SRR1)_variants.vcf)" >> $(CSV_OUTPUT)
	@echo "$(SRR2), $$(grep -c '^#' $(SRR2)_variants.vcf)" >> $(CSV_OUTPUT)
	@echo "$(SRR3), $$(grep -c '^#' $(SRR3)_variants.vcf)" >> $(CSV_OUTPUT)
